...
## Corpus Check
- 109 files · ~136,917 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2561 nodes · 5374 edges · 118 communities (112 shown, 6 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 449 edges (avg confidence: 0.52)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `761b7087`
...
Cohesion: 0.07
Nodes (54): int, str, _build_speech_attrs(), _cfg(), _cfg_int(), _deepgram_language(), es_numero_troncal_o_empresa(), extract_destination_address() (+46 more)
...
Cohesion: 0.06
Nodes (56): float, str, get_pusher_client(), str, trigger_pusher_event(), _clean_for_tts(), _edge_tts_sync_bytes(), get_voice_engine() (+48 more)

### Community 5 - "App Config & Logging"
Cohesion: 0.11
Nodes (34): ChatResponse, get_connection(), str, core/database.py — Pool de conexiones MySQL con PyMySQL.  Provee get_connectio, Yields a PyMySQL connection. Closes it on exit.     Allows specifying a differe, Execute a .sql file against the database., run_migration(), get_conversation_history() (+26 more)

### Community 6 - "Scheduling & Shared Utils"
Cohesion: 0.11
Nodes (35): Normaliza fecha/hora preferidas desde args del LLM o el historial reciente., _resolve_schedule_datetime(), build_schedule_clarification(), extract_session_today(), extract_session_user_id(), extract_tastes_from_history(), format_time_24h(), get_recent_user_messages() (+27 more)
...
Cohesion: 0.11
Nodes (28): _build_hint_vocab(), Construye la cadena de hints model-aware desde los catálogos (cacheada)., Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Limpia ruido conversacional al inicio de un transcript de dirección.      1. Q, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, repair_location_transcription() (+20 more)
...
Cohesion: 0.06
Nodes (43): clear_cache(), ConfigUpdate, create_version(), get_config(), _get_config_value(), _get_current_version(), get_status(), health_check() (+35 more)

### Community 12 - "Conversation Repair"
Cohesion: 0.06
Nodes (39): ConversationMemory, ConversationRepair, _extract_partial_location(), get_progressive_retry_message(), get_repair_message(), infer_intent(), bool, float (+31 more)
...
Cohesion: 0.08
Nodes (28): _build_schedule_clarification(), _extract_session_user_id(), _extract_tastes_from_history(), _find_anchored_id_in_messages(), _get_recent_user_messages(), _is_generic_query(), _match_property_id_in_reply(), _normalize() (+20 more)
...
Cohesion: 0.11
Nodes (23): ChatRequest, ChatService, float, Request, check_connection(), bool, Quick ping to verify MySQL is reachable., load_project_config() (+15 more)
...
Cohesion: 0.10
Nodes (26): Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, Convierte una referencia humana informal a datos estructurados.     Retorna dic, resolve_human_reference() (+18 more)

### Community 30 - "Context Builder & Prompt"
Cohesion: 0.10
Nodes (22): Path, str, bytes, int, str, bytes, str, ffmpeg_executable() (+14 more)
...
Cohesion: 0.06
Nodes (37): _build_dtmf_gather(), _generate_say_twiml(), _generate_tts_audio(), Construye un <Gather input="dtmf speech"> con menú numérico de barrios.      A, Construye un <Gather input="dtmf speech"> con menú numérico de barrios.      A, Construye un <Gather input="dtmf speech"> con menú numérico de barrios.      A, Construye un <Gather input="dtmf speech"> con menú numérico de barrios.      A, Construye un <Gather input="dtmf speech"> con menú numérico de barrios.      A (+29 more)
...
Cohesion: 0.14
Nodes (24): Any, bool, bytes, float, Request, WebSocket, BaseModel, Decision (+16 more)
...
Cohesion: 0.14
Nodes (18): _content_tokens(), _has_content(), _is_all_filler(), is_filler(), bool, float, str, Fracción de tokens de contenido del input que encuentran un equivalente     fuer (+10 more)
...
Cohesion: 0.16
Nodes (27): bool, bytes, float, int, _adaptive_threshold(), _detect_end(), detect_end_of_utterance(), detect_end_of_utterance_pcm16() (+19 more)
...
Cohesion: 0.16
Nodes (10): BaseSettings, str, core/config.py — Configuración centralizada desde .env con pydantic-settings., Base URL del endpoint LLM segun proveedor., Resolve model path relative to project root., Resolve model path relative to project root., Resolve model path relative to project root., Resolve model path relative to project root. (+2 more)

### Community 73 - "Community 73"
Cohesion: 0.12
Nodes (19): extract_destination_address(), extract_pickup_address(), looks_like_place(), Elimina saludos y relleno del inicio/fin del texto., Valida que `text` parezca una ubicación real en Popayán, para descartar     ext, Valida que `text` parezca una ubicación real en Popayán, para descartar     ext, Valida que `text` parezca una ubicación real en Popayán, para descartar     ext, Extrae dirección de recogida del texto libre.     Retorna (dirección, None) o ( (+11 more)
...
Cohesion: 0.14
Nodes (19): Quita tildes para comparaciones fonéticas., Quita tildes para comparaciones fonéticas., strip_accents(), AsyncClient, CallSession, float, str, _disambiguation_question() (+11 more)
...
Cohesion: 0.07
Nodes (31): bool, _get_contextual_hints(), Genera hints de vocabulario FOCALIZADOS según el estado (máx ~15 términos)., Genera hints de vocabulario FOCALIZADOS según el estado (máx ~15 términos)., Genera hints de vocabulario según el estado y el modelo STT.      - Captura (w, CallSession, Parámetros de endpointing adaptativos para el próximo Gather., Parámetros de endpointing adaptativos para el próximo Gather. (+23 more)

### Community 95 - "Community 95"
Cohesion: 0.06
Nodes (43): Request, _cache_audio(), _generate_play_twiml(), _get_base_url_for_twilio(), Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único. (+35 more)
...
Cohesion: 0.12
Nodes (16): _collapse_adjacent_duplicate_phrases(), correct_stt_errors(), _normalize_street_abbreviations(), Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., Normaliza abreviaturas de calle a forma canónica.     cl, cll, c/ → calle | cra, Normaliza abreviaturas de calle a forma canónica.     cl, cll, c/ → calle | cra (+8 more)
...
Cohesion: 0.20
Nodes (8): float, _max_utterance_sec(), Acumulador de audio WebSocket — segmentación por duración + VAD., Duración máxima de locución antes de forzar flush (env-driven)., Duración máxima de locución antes de forzar flush (env-driven)., Silencia la captura durante un playback y descarta lo ya acumulado., Silencia la captura durante un playback y descarta lo ya acumulado., Silencia la captura durante un playback y descarta lo ya acumulado.
...
Cohesion: 0.04
Nodes (73): AsyncClient, AudioQualityProfile, float, WebSocket, AsyncClient, ConversationMemory, BargeInHandler, Detecta y maneja interrupciones del usuario mientras Lyra habla.          Perm (+65 more)

### Community 109 - "Community 109"
Cohesion: 0.10
Nodes (23): _best_catalog_snap(), bigram_similarity(), _build_phonetic_repair_index(), combined_score(), fuzzy_match_location(), phonetic_key(), float, str (+15 more)
...
Cohesion: 0.06
Nodes (57): str, AudioEncoding, object, _echo_tokens(), _flush_audio_turn(), freeswitch_health(), _get_ws_buffer(), head_audio_file() (+49 more)
...
Cohesion: 0.27
Nodes (6): villa del norte' es un barrio; no debe convertirse en 'SENA Norte' por     la pa, villa del norte' es un barrio; no debe convertirse en 'SENA Norte' por     la p, Un substring cubierto (textual) debe ganar a cualquier casi-acierto     fonético, Un substring cubierto (textual) debe ganar a cualquier casi-acierto     fonétic, test_phonetic_does_not_override_substring(), test_villa_del_norte_not_routed_to_sena()
...
- **Why does `get_connection()` connect `App Config & Logging` to `Geocoding Types & Candidates`, `Community 106`, `Admin Config Management`, `Response Generation`, `Booking Flow Tests`, `Community 86`, `Community 89`, `API Dependency Injection`, `Navigation & UI Tools`?**
  _High betweenness centrality (0.085) - this node is a cross-community bridge._
- **Why does `FastAPI` connect `Community 106` to `Twilio Voice Router`, `Browser Voice & TTS`, `App Config & Logging`, `Colombian Address Extraction`, `Admin Config Management`, `Response Generation`, `Community 111`, `WhatsApp Channel Router`, `Community 83`, `Community 86`, `API Dependency Injection`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `resolve_location_entity()` connect `LLM Engine & Middleware` to `Twilio Voice Router`, `Community 98`, `Community 100`, `Community 73`, `Admin Session Management`, `Community 109`, `Community 111`, `Community 80`, `TTS Audio Router`, `Community 84`, `Community 118`, `IntelliTaxi Tools`, `Community 92`, `Adaptive Endpoint Control`?**
