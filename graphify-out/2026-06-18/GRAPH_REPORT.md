...
Cohesion: 0.06
Nodes (60): ResolutionStatus, get_async_openai_client(), Get or create the async OpenAI/OpenRouter client.     Uses AsyncOpenAI so that, Enum, Any, AsyncClient, bool, float (+52 more)
...
Cohesion: 0.11
Nodes (17): ConversationMemory, ConversationRepair, get_repair_message(), bool, float, str, Genera respuestas de reparación inteligentes cuando el STT falla o     el texto, Rota entre templates para no repetir siempre la misma frase. (+9 more)

### Community 13 - "Voice Session State"
Cohesion: 0.12
Nodes (31): AudioQualityProfile, bytes, float, int, Request, WebSocket, BargeInHandler, Detecta y maneja interrupciones del usuario mientras Lyra habla.          Perm (+23 more)
...
Cohesion: 0.09
Nodes (34): SessionUpdate, get_chat_service(), get_db(), get_llm(), get_settings(), get_tool_registries(), get_twilio_service(), get_whatsapp_service() (+26 more)

### Community 17 - "Tool Adapter & Registry"
Cohesion: 0.11
Nodes (23): bool, str, _echo_tokens(), _flush_audio_turn(), _inbound_use_file_mode(), _looks_like_bot_echo(), _playback_tts_on_call(), True si el transcript es un fragmento literal del último mensaje del bot. (+15 more)
...
Cohesion: 0.12
Nodes (10): AdaptiveEndpointController, AudioQualityProfile, bool, float, Estima palabras por segundo basado en el texto acumulado y el tiempo.         Ú, Adapta los parámetros de VAD/endpointing de Twilio basado en:     - Perfil de a, Retorna los parámetros óptimos para el próximo <Gather>.                  stat, Procesa el texto final de STT de un turno.                  Retorna: (+2 more)

### Community 20 - "Tool Runner Utilities"
Cohesion: 0.08
Nodes (30): _build_schedule_clarification(), _extract_session_user_id(), _extract_tastes_from_history(), _find_anchored_id_in_messages(), _get_recent_user_messages(), _is_generic_query(), _match_property_id_in_reply(), _normalize() (+22 more)

### Community 21 - "LLM Engine & Middleware"
Cohesion: 0.07
Nodes (49): Búsqueda local en el catálogo de barrios/landmarks de Popayán     (popayan_geod, Búsqueda local en el catálogo de barrios/landmarks de Popayán     (popayan_geod, _try_local_match(), _base_decision(), decide(), LocationMatch, Decisión por tipo+confianza, SIN considerar desambiguación. Una     coincidenci, Mapea un LocationMatch a la acción del flujo. Precisión sobre recall.      La (+41 more)
...
Cohesion: 0.15
Nodes (27): _build_local_match_index(), _clean_stt_text(), _compound_num_replace(), _correct_speech(), extract_datetime_local(), extract_datetime_with_llm(), _geocode_cache_get(), _geocode_cache_set() (+19 more)
...
Cohesion: 0.04
Nodes (65): _build_dtmf_gather(), _cache_audio(), _generate_play_twiml(), _generate_say_twiml(), _generate_tts_audio(), _get_base_url_for_twilio(), Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único. (+57 more)
...
Cohesion: 0.15
Nodes (15): int, Request, RateLimitMiddleware, gateway/middleware.py — Rate limiting en memoria (dict + timestamp).  Simple i, Per-IP rate limiter.     max_requests: maximum requests per window.     window, BaseHTTPMiddleware, FastAPI, lifespan() (+7 more)
...
Cohesion: 0.33
Nodes (5): BaseSettings, str, Resolve model path relative to project root., Resolve model path relative to project root., Settings

### Community 47 - "Project Configurations"
Cohesion: 0.09
Nodes (30): delete_session(), export_sessions(), list_sessions(), Export sessions (basic JSON export)., Get full session with messages., Delete a session and its messages., List conversations (sessions) with pagination., Session aggregate stats. (+22 more)
...
Cohesion: 0.10
Nodes (34): _best_for_entity(), _build_catalog(), _content_tokens(), _Entity, _has_content(), _is_all_filler(), is_filler(), _key() (+26 more)

### Community 52 - "Barge-In Interruption"
Cohesion: 0.18
Nodes (19): AudioEncoding, bool, bytes, float, int, str, detect_end_of_utterance(), detect_end_of_utterance_pcm16() (+11 more)
...
Cohesion: 0.04
Nodes (83): str, _build_speech_attrs(), _cfg(), _cfg_int(), _deepgram_language(), _disambiguation_question(), es_numero_troncal_o_empresa(), extract_address() (+75 more)
...
Cohesion: 0.18
Nodes (8): _get_ws_buffer(), bool, bytes, float, int, Silencia la captura durante un playback y descarta lo ya acumulado., WsAudioBuffer, WsAudioBuffer
...
Cohesion: 0.09
Nodes (22): ConversationMemory, generate_contextual_response(), PartialTranscript, int, str, Mejor texto disponible (final si existe, si no el último parcial)., Segundos desde la última actividad de voz., Procesa texto parcial y retorna estado de intención actualizado. (+14 more)

### Community 76 - "Community 76"
Cohesion: 0.29
Nodes (6): LLMEngine, float, int, str, Parse a failed_generation string like <function=name>{"k":v}</function>, Fix type mismatches: cast int→str, null→default, unwrap 'properties' wrapper.
...
Cohesion: 0.15
Nodes (14): extract_destination_address(), extract_pickup_address(), looks_like_place(), normalize_colombian_address(), Normaliza al formato colombiano estándar.     'carrera cuarta a el # 17 b 28' →, Normaliza al formato colombiano estándar.     'carrera cuarta a el # 17 b 28' →, Valida que `text` parezca una ubicación real en Popayán, para descartar     ext, Valida que `text` parezca una ubicación real en Popayán, para descartar     ext (+6 more)

### Community 79 - "Community 79"
Cohesion: 0.18
Nodes (10): head_audio_file(), Sirve WAV 8 kHz mono generado para playback en FreeSWITCH.     URL ejemplo: /fr, Sirve WAV 8 kHz mono generado para playback en FreeSWITCH.     URL ejemplo: /fr, HEAD para que FreeSWITCH valide existencia del WAV sin descargar cuerpo., HEAD para que FreeSWITCH valide existencia del WAV sin descargar cuerpo., Sirve WAV 8 kHz mono generado para playback en FreeSWITCH.     URL ejemplo: /fr, Sirve WAV 8 kHz mono generado para playback en FreeSWITCH.     URL ejemplo: /fr, HEAD para que FreeSWITCH valide existencia del WAV sin descargar cuerpo. (+2 more)
...
Cohesion: 0.25
Nodes (8): AsyncClient, AsyncClient, _create_service(), Geocodifica y crea el servicio de taxi en el backend Laravel., Geocodifica y crea el servicio de taxi en el backend Laravel., Geocodifica y crea el servicio de taxi en el backend Laravel., Geocodifica y crea el servicio de taxi en el backend Laravel., Geocodifica y crea el servicio de taxi en el backend Laravel.

### Community 84 - "Community 84"
Cohesion: 0.15
Nodes (24): float, Request, freeswitch_health(), inbound_call(), InboundCallRequest, _normalize_transcript(), _parse_inbound_body(), process_text() (+16 more)
...
Cohesion: 0.36
Nodes (5): bytes, str, Genera audio de respuesta para llamadas telefónicas., Genera audio listo para telefonía.          Returns:             {, TelephonyTTSService

### Community 87 - "Community 87"
Cohesion: 0.10
Nodes (15): AudioQualityProfile, float, int, Perfil de calidad de audio de una llamada.     Se actualiza turn a turn para ad, Perfil de calidad de audio de una llamada.     Se actualiza turn a turn para ad, Recomienda el speechTimeout de Twilio basado en el perfil del usuario., Perfil de calidad de audio de una llamada.     Se actualiza turn a turn para ad, Timeout total de <Gather> en segundos. (+7 more)
...
Cohesion: 0.33
Nodes (6): _aggressive_normalize(), Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli

### Community 92 - "Community 92"
Cohesion: 0.05
Nodes (59): _extract_partial_location(), infer_intent(), core/conversation_repair.py — Motor de reparación conversacional inteligente par, Infiere la intención del usuario con scores de probabilidad.          Retorna:, Extrae cualquier fragmento que parezca un lugar (aún si el texto es incompleto)., _prioritized_canonical_names(), core/streaming_pipeline.py — Pipeline de streaming incremental para Lyra.  Imp, Nombres canónicos del catálogo, con los más relevantes al frente. (+51 more)

### Community 93 - "Community 93"
Cohesion: 0.29
Nodes (6): bool, Parámetros de endpointing adaptativos para el próximo Gather., Parámetros de endpointing adaptativos para el próximo Gather., Parámetros de endpointing adaptativos para el próximo Gather., Parámetros de endpointing adaptativos para el próximo Gather., Parámetros de endpointing adaptativos para el próximo Gather.
...
Cohesion: 0.33
Nodes (6): bytes, Acumula audio y dispara STT al alcanzar duración mínima + silencio o máximo., Acumula audio y dispara STT al alcanzar duración mínima + silencio o máximo., Acumula audio y dispara STT al alcanzar duración mínima + silencio o máximo., Acumula audio y dispara STT al alcanzar duración mínima + silencio o máximo., _ws_append_audio()
...
- **Why does `FastAPI` connect `Chat Service` to `Community 100`, `App Config & Logging`, `Community 70`, `Admin Session Management`, `Admin Config Management`, `Community 76`, `Project Configurations`, `Main Router & Database`, `WhatsApp Channel Router`, `Community 84`?**
  _High betweenness centrality (0.095) - this node is a cross-community bridge._
- **Why does `get_connection()` connect `Project Configurations` to `Geocoding Types & Candidates`, `App Config & Logging`, `Nexiservice Tools`, `Admin Config Management`, `Booking Flow Tests`, `Main Router & Database`, `Response Engine & Templates`?**
  _High betweenness centrality (0.072) - this node is a cross-community bridge._
- **Why does `ToolRegistry` connect `Chat Service` to `Community 72`, `Colombian Address Extraction`, `Tool Runner Utilities`, `Streaming STT Pipeline`?**
