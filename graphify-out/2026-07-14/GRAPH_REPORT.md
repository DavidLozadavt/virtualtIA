...
## Corpus Check
- 109 files · ~136,157 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2520 nodes · 5316 edges · 112 communities (105 shown, 7 thin omitted)
...
Cohesion: 0.06
Nodes (63): int, str, _build_dtmf_gather(), _build_speech_attrs(), _cfg(), _cfg_int(), _deepgram_language(), es_numero_troncal_o_empresa() (+55 more)

### Community 1 - "Geocoding Types & Candidates"
Cohesion: 0.17
Nodes (29): GeoCandidate, Solo ROOFTOP y RANGE_INTERPOLATED dentro del área urbana se auto-aceptan., _db_set(), _extract_neighborhood_google(), _google_autocomplete(), _google_autocomplete_candidates(), _google_get_candidates(), _google_place_details() (+21 more)
...
Cohesion: 0.09
Nodes (42): Normaliza fecha/hora preferidas desde args del LLM o el historial reciente., _resolve_schedule_datetime(), build_schedule_clarification(), extract_session_today(), extract_session_user_id(), extract_tastes_from_history(), find_anchored_id_in_messages(), format_time_24h() (+34 more)
...
Cohesion: 0.09
Nodes (36): Any, AsyncClient, bool, CallSession, float, str, int, str (+28 more)

### Community 9 - "Admin Session Management"
Cohesion: 0.20
Nodes (16): _build_hint_vocab(), Construye la cadena de hints model-aware desde los catálogos (cacheada)., Limpia ruido conversacional al inicio de un transcript de dirección.      1. Q, strip_conversational_prefix(), tests/test_stt_repair.py — Cobertura de la capa STT anterior al resolver: repar, test_deepgram_hints_are_single_tokens(), test_deepgram_hints_drop_generic_words(), test_deepgram_hints_include_key_places() (+8 more)
...
Cohesion: 0.09
Nodes (28): clear_cache(), ConfigUpdate, create_version(), _get_current_version(), get_status(), health_check(), health_incidents(), mark_alert_read() (+20 more)

### Community 12 - "Conversation Repair"
Cohesion: 0.07
Nodes (32): ConversationMemory, ConversationRepair, _extract_partial_location(), get_progressive_retry_message(), get_repair_message(), infer_intent(), bool, float (+24 more)

### Community 13 - "Voice Session State"
Cohesion: 0.05
Nodes (46): AdaptiveEndpointController, generate_contextual_response(), _get_contextual_hints(), PartialTranscript, _prioritized_canonical_names(), bool, float, int (+38 more)
...
Cohesion: 0.17
Nodes (12): _build_phonetic_repair_index(), Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, repair_location_transcription(), test_repair_does_not_corrupt_normal_speech(), test_repair_empty_and_short() (+4 more)
...
Cohesion: 0.20
Nodes (7): str, Base URL del endpoint LLM segun proveedor., Resolve model path relative to project root., Resolve model path relative to project root., Resolve model path relative to project root., Resolve model path relative to project root., Credencial para llamadas de chat al LLM.          OpenRouter y OpenAI usan sis

### Community 20 - "Tool Runner Utilities"
Cohesion: 0.06
Nodes (51): ToolRegistry, _build_schedule_clarification(), _extract_session_today(), _extract_session_user_id(), _extract_tastes_from_history(), _find_anchored_id_in_messages(), _get_recent_user_messages(), _inject_ids_into_titles() (+43 more)

### Community 21 - "LLM Engine & Middleware"
Cohesion: 0.06
Nodes (59): _base_decision(), decide(), LocationMatch, Decisión por tipo+confianza, SIN considerar desambiguación. Una     coincidenci, Mapea un LocationMatch a la acción del flujo. Precisión sobre recall.      La, Resuelve `text` a una entidad del catálogo con tipo y confianza.      scope: lis, Resuelve `text` a una entidad del catálogo con tipo y confianza.      scope: lis, Resuelve `text` a una entidad del catálogo con tipo y confianza.      scope: l (+51 more)
...
Cohesion: 0.20
Nodes (10): correct_stt_errors(), Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., Aplica correcciones STT en orden de especificidad:     1. Match exacto     2., test_correct_full_chain_no_corruption(), test_correct_hortigal_no_word_duplication(), test_correct_leaves_normal_speech_unchanged(), test_correct_popayan_hortigal_clean() (+2 more)
...
Cohesion: 0.12
Nodes (20): bool, float, int, str, float, GeographicMatcher, _haversine(), _is_in_popayan() (+12 more)
...
Cohesion: 0.09
Nodes (30): AsyncOpenAI, call_llm(), call_llm_async(), extract_json_object(), get_async_openai_client(), get_model(), get_openai_client(), Any (+22 more)

### Community 37 - "SENA Interceptors"
Cohesion: 0.22
Nodes (8): int, Request, RateLimitMiddleware, Per-IP rate limiter.     max_requests: maximum requests per window.     window, BaseHTTPMiddleware, Request, validation_exception_handler(), RequestValidationError

### Community 38 - "Audio Quality Profiling"
Cohesion: 0.22
Nodes (8): True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno)., True si el usuario habla en frases largas (muchas palabras por turno).

### Community 39 - "Streaming Pipeline Entry"
Cohesion: 0.06
Nodes (50): _get_config_value(), gateway/middleware.py — Rate limiting en memoria (dict + timestamp).  Simple i, float, str, BaseModel, BaseSettings, core/config.py — Configuración centralizada desde .env con pydantic-settings., Settings (+42 more)
...
Cohesion: 0.13
Nodes (14): AudioEncoding, _get_ws_buffer(), bool, bytes, float, int, _max_utterance_sec(), Acumulador de audio WebSocket — segmentación por duración + VAD. (+6 more)
...
Cohesion: 0.06
Nodes (52): _best_for_entity(), _build_catalog(), catalog_terms(), _content_tokens(), _Entity, _has_content(), _is_all_filler(), is_filler() (+44 more)
...
Cohesion: 0.14
Nodes (28): bool, bytes, float, int, str, _adaptive_threshold(), _detect_end(), detect_end_of_utterance() (+20 more)

### Community 53 - "Booking State"
Cohesion: 0.26
Nodes (14): LocationType, core/geo_types.py — Tipos del pipeline de geocodificación.  Separación de resp, ResolutionStatus, _db_get(), _google_location_type(), _pred_precision(), float, LocationType (+6 more)
...
Cohesion: 0.09
Nodes (29): Elimina saludos y relleno del inicio/fin del texto., _strip_preamble(), _build_context_question(), _build_disambiguation_question(), _build_house_number_question(), handle_user_context(), _match_user_to_candidate(), int (+21 more)
...
Cohesion: 0.22
Nodes (8): True si el usuario usa frases muy cortas., True si el usuario usa frases muy cortas., Repara la grafía de nombres de lugar en `text` usando el catálogo, vía     simi, True si el usuario usa frases muy cortas., True si el usuario usa frases muy cortas., True si el usuario usa frases muy cortas., True si el usuario usa frases muy cortas., True si el usuario usa frases muy cortas.
...
Cohesion: 0.04
Nodes (66): Any, bool, bytes, str, WebSocket, audio_stream(), _echo_tokens(), _flush_audio_turn() (+58 more)
...
Cohesion: 0.33
Nodes (6): _extract_geo_context(), Extrae el término geográfico útil del texto libre del usuario para enrichment., Extrae el término geográfico útil del texto libre del usuario para enrichment., Extrae el término geográfico útil del texto libre del usuario para enrichment., Extrae el término geográfico útil del texto libre del usuario para enrichment., Extrae el término geográfico útil del texto libre del usuario para enrichment.
...
Cohesion: 0.12
Nodes (17): float, _aggressive_normalize(), preprocess_stt(), Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli, Normalización agresiva previa, para audio telefónico muy degradado.      - Eli (+9 more)

### Community 92 - "Community 92"
Cohesion: 0.06
Nodes (50): Decision, _best_catalog_snap(), bigram_similarity(), combined_score(), fuzzy_match_location(), _is_street(), phonetic_key(), bool (+42 more)
...
Cohesion: 0.06
Nodes (34): bool, CallSession, get_session(), _prune_sessions(), partialResultCallback (/partial_speech) — DESACTIVADO por defecto.      Razón:, partialResultCallback (/partial_speech) — DESACTIVADO por defecto.      Razón:, partialResultCallback (/partial_speech) — DESACTIVADO por defecto.      Razón:, partialResultCallback (/partial_speech) — DESACTIVADO por defecto.      Razón: (+26 more)

### Community 95 - "Community 95"
Cohesion: 0.03
Nodes (71): bytes, _cache_audio(), _generate_play_twiml(), _generate_say_twiml(), _generate_tts_audio(), Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único., Almacena audio en cache y retorna un ID único. (+63 more)
...
Cohesion: 0.67
Nodes (3): _collapse_adjacent_duplicate_phrases(), Colapsa frases idénticas adyacentes:       "valle del valle del ortigal" → "val, test_collapse_adjacent_duplicate_phrases()
...
Cohesion: 0.33
Nodes (9): _context(), _engine(), tests/test_voice_engine_landmark.py  Cubre el cierre del gap en _handle_waiting_, _resolved(), _session(), test_la_paz_first_turn_asks_disambiguation_without_geocoding(), test_trusted_landmark_coarse_falls_to_plain_confirm(), test_trusted_landmark_resolved_confirms() (+1 more)
...
Cohesion: 0.09
Nodes (27): get_config(), list_versions(), Get Lyra configuration., List Lyra version history., delete_session(), export_sessions(), list_sessions(), Export sessions (basic JSON export). (+19 more)

### Community 108 - "Community 108"
Cohesion: 0.05
Nodes (51): AsyncClient, AudioQualityProfile, WebSocket, AsyncClient, ConversationMemory, BargeInHandler, Detecta y maneja interrupciones del usuario mientras Lyra habla.          Perm, Detecta y maneja interrupciones del usuario mientras Lyra habla.          Perm (+43 more)
...
Cohesion: 0.08
Nodes (41): float, Request, object, freeswitch_health(), head_audio_file(), inbound_call(), InboundCallRequest, _normalize_transcript() (+33 more)

### Community 112 - "Community 112"
Cohesion: 0.13
Nodes (17): GeoResolution, in_urban_bbox(), in_wide_bbox(), bool, float, FreeSWITCH: cuando run_pipeline devuelve FAILED (intentos agotados, aún baja, test_freeswitch_geo_context_exhaustion_barrio_handoff(), test_freeswitch_geo_context_exhaustion_triggers_safe_fallback() (+9 more)

### Community 116 - "Community 116"
Cohesion: 0.07
Nodes (29): Request, Response, partial_speech(), Webhook inicial de Twilio. Saluda y comienza a escuchar., Webhook inicial de Twilio. Saluda y comienza a escuchar., Webhook inicial de Twilio. Saluda y comienza a escuchar., Webhook inicial de Twilio. Saluda y comienza a escuchar., Webhook inicial de Twilio. Saluda y comienza a escuchar. (+21 more)
...
- **Why does `FastAPI` connect `Streaming Pipeline Entry` to `Twilio Voice Router`, `SENA Interceptors`, `Community 106`, `Admin Config Management`, `Community 76`, `Community 111`, `WhatsApp Channel Router`, `Community 83`, `Tool Runner Utilities`, `Community 86`, `API Dependency Injection`?**
  _High betweenness centrality (0.092) - this node is a cross-community bridge._
- **Why does `get_connection()` connect `Community 106` to `Community 128`, `Geocoding Types & Candidates`, `App Config & Logging`, `Streaming Pipeline Entry`, `Admin Config Management`, `Community 76`, `Booking Flow Tests`, `Booking State`, `Community 89`, `API Dependency Injection`, `Navigation & UI Tools`?**
  _High betweenness centrality (0.082) - this node is a cross-community bridge._
- **Why does `Decision` connect `Community 92` to `Twilio Voice Router`, `Nexiservice Booking Interceptor`, `Admin Session Management`, `Community 75`, `Community 108`, `Community 111`, `Project Configurations`, `TTS Audio Router`, `Community 116`, `LLM Engine & Middleware`, `Booking State`, `Community 87`, `IntelliTaxi Tools`, `Community 91`, `Community 94`, `Community 95`?**
